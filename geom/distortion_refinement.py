...
def refine_distortion_brown(
    ...
    """
    if len(points_3d) < 10:
        raise ValueError(f"Need at least 10 point correspondences for distortion, got {len(points_3d)}")
    
    logger.info(f"Refining Brown-Conrady distortion from {len(points_3d)} correspondences")
    ...
    if abs(k1) > 1.0 or abs(k2) > 0.5 or abs(k3) > 0.3:
        confidence *= 0.7
        logger.warning(f"Extreme distortion coefficients: k1={k1:.4f}, k2={k2:.4f}, k3={k3:.4f}")
...
def refine_distortion_auto(
    ...
    if projection_lower in ['spherical', 'equirectangular']:
        ...
        return refine_distortion_fisheye(
            points_3d, points_2d, focal, camera_pose, principal_point,
            image_size, initial_coeffs, optimize_focal
        )
...
def iterative_refinement(
    ...
    for iteration in range(max_iterations):
        ...
        focal_result = refine_focal_geometric(
            points_3d, points_2d, current_focal, camera_pose,
            principal_point, image_size, current_coeffs
        )
        ...
        dist_result = refine_distortion_auto(
            points_3d, points_2d, current_focal, camera_pose,
            principal_point, image_size, projection_type,
            current_coeffs, optimize_focal=False
        )
