...
def validate_intrinsics(
    ...
    if focal is None:
        ...
        if focal < expected_focal_range[0]:
            warnings.append(f"Focal length {focal:.3f} unusually low (< {expected_focal_range[0]})")
            focal_status = 'suspect'
            needs_refinement = True
            confidence *= 0.7
        elif focal > expected_focal_range[1]:
            warnings.append(f"Focal length {focal:.3f} unusually high (> {expected_focal_range[1]})")
            focal_status = 'suspect'
            needs_refinement = True
            confidence *= 0.7
        ...
        if abs(focal - round(focal, 1)) < 1e-6 and focal != 0.5:
            warnings.append(f"Focal length {focal} is suspiciously round - may be approximate")
            confidence *= 0.9
    ...
    if focal_y is not None:
        if focal is not None:
            ...
            if abs(aspect_ratio - 1.0) > 0.05:  # More than 5% difference
                warnings.append(f"Focal aspect ratio {aspect_ratio:.3f} deviates from 1.0")
                confidence *= 0.95
    ...
    distortion_present = {k: camera.get(k) for k in distortion_params if camera.get(k) is not None}
    ...
    if not distortion_present:
        ...
        for coef_name, coef_val in distortion_present.items():
            ...
            if abs(coef_val) > threshold:
                warnings.append(f"Distortion {coef_name}={coef_val:.4f} exceeds typical range (|{coef_name}| < {threshold})")
        ...
        if all(abs(v) < 1e-9 for v in distortion_present.values()):
            warnings.append("All distortion coefficients are zero - may be uninitialized")
            distortion_status = 'suspect'
            needs_refinement = True
            confidence *= 0.7
    ...
    # Overall validation
    valid = (
        focal_status in ['good', 'missing'] and
        pp_status in ['good', 'missing'] and
        distortion_status in ['good', 'missing']
    )
    
    if not valid:
        warnings.append("Camera parameters have suspicious values - refinement recommended")
...
def validate_sequence_consistency(
    ...
    if focal_std > 0:
        ...
        if outliers:
            ...
            consistent = False
            stats["outliers"] = [(img_id, float(f), float(z)) for img_id, f, z in outliers[:5]]
...
def summarize_validation(
    ...
    sequence_consistency: Dict[str, any]
) -> Dict[str, any]:
