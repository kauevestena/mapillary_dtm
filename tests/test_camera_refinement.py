...
def test_validate_intrinsics_good_perspective():
    ...
    camera = {
        ...
        'height': 1080,
        'focal': 0.85,
        'principal_point': [0.501, 0.499],  # Slightly off-center to avoid default warning
        'k1': -0.15,
        'k2': 0.03,
        'p1': 0.001,
        'p2': -0.001
...
def test_validate_intrinsics_zero_distortion():
    ...
    assert result.distortion_status == 'suspect'
    assert any("all distortion coefficients are zero" in w.lower() for w in result.warnings)
