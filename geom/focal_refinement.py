...
def refine_focal_geometric(
    ...
    def reprojection_error(focal: float) -> float:
        ...
        py = (
            focal * width * proj_y + principal_point[1] * height
        )  # Use width for aspect
...
def refine_focal_ransac(
    ...
    for _ in range(ransac_iterations):
        ...
        for i in range(len(sample_3d)):
            if sample_3d[i, 2] > 0 and abs(sample_2d[i, 0]) > 1e-6:
                ...
                f_est = (
                    (
                        sample_2d[i, 0] * image_size[0]
                        - principal_point[0] * image_size[0]
                    )
                    / (sample_3d[i, 0] / sample_3d[i, 2])
                    / image_size[0]
                )
    ...
    if best_inlier_mask is not None and best_inliers >= 6:
        ...
        result = minimize_scalar(
            inlier_error, bounds=(best_focal * 0.8, best_focal * 1.2), method="bounded"
        )
        refined_focal = result.x
    else:
    ...
    confidence = 0.7 * inlier_ratio + 0.3 * np.clip(
        improvement / (initial_error + 1e-6), 0, 1
    )
...
def apply_distortion(
    x: np.ndarray, y: np.ndarray, coeffs: Dict[str, float]
...
def refine_focal_bundle_adjustment(
    ...
    logger.warning(
        "Bundle adjustment is placeholder - requires SfM backend integration"
    )
