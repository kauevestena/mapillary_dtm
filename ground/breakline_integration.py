        ...
        for curb in curb_list:
            ...
            for x_norm, y_norm in curb.xy_norm:
                ...
                if tree is not None and len(consensus_xy) > 0:
                    ...
                    dists_pt, indices_pt = tree.query([X, Y], k=min(5, len(consensus_xy)))
...
def merge_breakline_segments(
    ...
    log.info("Filtered %d/%d breaklines by minimum length %.2fm",
             len(filtered), len(breaklines), min_length)
...
def simplify_breaklines(
    ...
    log.info("Simplified breaklines: %d → %d vertices (%.1f%% reduction)",
             total_before, total_after, 100 * (1 - total_after / max(total_before, 1)))
...
def densify_breaklines(
    ...
    log.info("Densified breaklines: %d vertices, %d edges", len(vertices), len(all_edges))
...
def _merge_polylines(polylines: List[List[Tuple[float, float, float]]]) -> List[Tuple[float, float, float]]:
